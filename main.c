...
// Timecode for smae-speed movements regardless of framerate (probably doesn't
// work as well as described though)
float _dt = 0.0f;

// Values for if the application is running and focused, so no rendering when
// not in focus basically
...
#endif
{
  ...
  // Capture the cursor and keep it centered to properly calculate relative
  // mouse position
  ...
  // GL loader more specifically for Windows that fails to expose all OpenGL
  // functions Similar to GLEW, just simpler, see
  // https://github.com/Dav1dde/glad Generated files are included in the "glad"
  // directory
  gladLoadGL();
  ...
  while (running) {
    ...
    if (active) {
      ...
      // Update the starting timecode so the world doesn't end when the
      // application comes back into focus
      start = SDL_GetTicks();
      lt = 0;

      // Wait longer, because the CPU shoudln't work for something like this
      // that's not being used
      SDL_Delay(100);
    }

    // Reset mouse movement, offset and wheel offset values at the end of the
    // loop
    _mm = 0;
    _mx = _my = _mw = 0.0f;
  }
...
// So it'll return false after the first call until the key is released and
// pressed again This can cause issues in some places, but luckily I don't use
// it too much here
byte kp(unshort k) {
  if (_ki[k] && !_kp[k])
    return (_kp[k] = 1);
