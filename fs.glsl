// Fragment shader, this is where all the "texmode" references will start making
// sense Also, sorry that everything is in one file, I was in a hurry to get
// results, time was short when I wrote this, as it is now
#version 330 core
...
void main(void) {
  ...
  case 1:
    output_col *= texture(texmap, vtc);
    break;
  // Stained glass effect (warping texture coords according to the provided
  // noise texture)
  case 2:
    output_col *= stained_glass();
    break;
  // Floor reflection
  case 3:
    output_col = floor_reflection();
    break;
  // Not sure what I was trying to do here, this mode is used in app.c
  case 4:
    output_col.rgb = vec3(1.0f - (vpos.z / 1.0f));
  }

  // Discard fragment if alhpa is 0
  if (output_col.a == 0.0f)
    discard;
}
