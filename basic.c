// This file sets up the basic shaders being used (even though onyl one set of
// shaders are being used)
#include "basic.h"

// Poorly-named constant of texcoord offste in main VBO (see closer to the end
// of this file)
...
void send_attribs(void) {
  ...
  glVertexAttribPointer(_basic_pv.tc, 2, GL_FLOAT, GL_FALSE, stride(),
                        _basic_offset);
}
...
void texmode(int mode) { glUniform1i(_basic_pv.texmode, mode); }
