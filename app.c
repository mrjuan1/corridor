...
void init(void) {
  ...
  if (init_vbo()) {
    // Load models
    const char *model_fnames[6] = {"corridor-ceiling.bin",
                                   "corridor-floor.bin",
                                   "corridor-wall.bin",
                                   "door.bin",
                                   "room.bin",
    ...
    if (i == 6) {
      ...
      // Load giant texture map generated in Blender and continue if
      // successfull...
      ...
      if (load_tex(textures, "textures.data", 2048, 2048, GL_RGB, tf_mipmap)) {
        ...
        // Change the "texture mode", see the fragment shaders for more info on
        // this
        texmode(1);
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, sw() / 2, sh() / 2, 0, GL_RGB,
                     GL_UNSIGNED_BYTE, (const void *)noise_data);
...
void loop(void) {
  ...
  if (mm()) {
    ...
    if (tilt < -89.999f)
      tilt = -89.999f;
    else if (tilt > 89.999f)
      tilt = 89.999f;
  }

  // If one of the directional keys are being pressed (asses WSAD for righty's)
  if (ki(SDL_SCANCODE_UP) || ki(SDL_SCANCODE_DOWN) || ki(SDL_SCANCODE_LEFT) ||
      ki(SDL_SCANCODE_RIGHT) || ki(SDL_SCANCODE_W) || ki(SDL_SCANCODE_S) ||
      ki(SDL_SCANCODE_A) || ki(SDL_SCANCODE_D)) {
    const float spd = 5.0f * dt();
    float t = dir, r = 0.0f;

    if ((ki(SDL_SCANCODE_UP) || ki(SDL_SCANCODE_W)) &&
        (ki(SDL_SCANCODE_LEFT) || ki(SDL_SCANCODE_A)))
      t += 45.0f;
    else if ((ki(SDL_SCANCODE_UP) || ki(SDL_SCANCODE_W)) &&
             (ki(SDL_SCANCODE_RIGHT) || ki(SDL_SCANCODE_D)))
      t -= 45.0f;
    else if ((ki(SDL_SCANCODE_DOWN) || ki(SDL_SCANCODE_S)) &&
             (ki(SDL_SCANCODE_LEFT) || ki(SDL_SCANCODE_A)))
      t += 135.0f;
    else if ((ki(SDL_SCANCODE_DOWN) || ki(SDL_SCANCODE_S)) &&
             (ki(SDL_SCANCODE_RIGHT) || ki(SDL_SCANCODE_D)))
      ...
      t += 180.0f;
    else if (ki(SDL_SCANCODE_LEFT) || ki(SDL_SCANCODE_A))
      t += 90.0f;
    else if (ki(SDL_SCANCODE_RIGHT) || ki(SDL_SCANCODE_D))
      t -= 90.0f;
  ...
  // Draw to floor "distance" framebuffer (I don't quite remember why this
  // exists)
  use_fbo(&floor_dist);
  clear();
  texmode(4);
